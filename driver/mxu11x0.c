...
#define MXU1_DRIVER_AUTHOR	"Jason Chen"
...
// Check if normal or boot firmware is running?
// Returns:
//   < 0 Got some errors
//   = 0 running normal firmware
//   = 1 running boot firmware
static int _chk_running_fw(struct usb_device *udev)
{
	...
	for( i=0; i<retry_cnt; i++ ){
		...
		status = usb_get_descriptor( udev, USB_DT_CONFIG, 1, desc, USB_DT_CONFIG_SIZE );
		
		//dbg("Get config value=%d, status=%d (%d)\n",  desc->bConfigurationValue, status, i);
		
		if( status<0 ){
			continue;
			//return status;
		} else if( status<4 ){
			dev_err(ddev, "Config index descriptor too short");
			kfree(desc);
			return -EINVAL;
		}
		
		if( desc->bConfigurationValue==2 )
		{
			//We have got the correct configuration
			kfree(desc);
			return 0;	
		}
	}
	...
	struct serial_struct __user *new_arg)
{
	...
	mxport->mxp_flags = new_arg->flags & MXU1_SET_SERIAL_FLAGS;
	mxport->mxp_closing_wait = new_arg->closing_wait;
